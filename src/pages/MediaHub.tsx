...
// Fallback thumbnail map for local assets
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-8 pt-10">
        <Link to="/" className="inline-flex items-center gap-2 font-mono text-[10px] tracking-[0.2em] uppercase text-muted-foreground hover:text-foreground transition-colors">
          ...
          {videos.map((video: any) => (
            <Link key={video.id} to={makeEpisodeLink(video)} className="group block border border-border bg-card overflow-hidden hover:shadow-lg transition-all duration-300">
              <div className="relative aspect-video overflow-hidden">
                <img src={resolveThumb(video.thumbnail_url)} alt={video.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ...
                {video.is_featured && (
                  <div className="absolute top-3 left-3">
                    <span className="bg-primary text-primary-foreground font-mono text-[8px] tracking-[0.15em] px-2 py-1 uppercase">Featured</span>
                  </div>
                )}
      ...
      <section className="max-w-6xl mx-auto px-8 py-16">
        <div className="flex items-center justify-between mb-10">
          <div>
            <p className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-2">Latest Episodes</p>
            <h2 className="font-serif text-3xl md:text-4xl tracking-tight" style={{ fontStyle: "italic", fontWeight: 400 }}>Podcast</h2>
          </div>
          ...
          {podcasts.map((pod: any) => (
            <Link key={pod.id} to={makeEpisodeLink(pod)} className="group flex border border-border bg-card overflow-hidden hover:shadow-lg transition-all duration-300">
              <div className="w-40 md:w-48 shrink-0 overflow-hidden">
                <img src={resolveThumb(pod.thumbnail_url)} alt={pod.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
      ...
      {/* UPCOMING — show latest unpublished or a static teaser */}
      <section className="max-w-6xl mx-auto px-8 py-16">
        <div className="text-center mb-10">
          <p className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-2">Coming Soon</p>
          <h2 className="font-serif text-3xl md:text-4xl tracking-tight" style={{ fontStyle: "italic", fontWeight: 400 }}>Next Episode</h2>
        </div>
        <div className="max-w-2xl mx-auto border border-border bg-card overflow-hidden">
          <div className="relative aspect-video overflow-hidden">
            <img src={thumbV005} alt="Upcoming episode" className="w-full h-full object-cover blur-sm scale-105" />
            <div className="absolute inset-0 bg-background/60 backdrop-blur-sm flex flex-col items-center justify-center">
              <Clock className="h-8 w-8 text-muted-foreground mb-3" />
              <p className="font-serif text-2xl md:text-3xl tracking-tight" style={{ fontStyle: "italic", fontWeight: 400 }}>Coming Soon</p>
              <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-muted-foreground mt-2">Stay tuned for the next episode</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-8 py-20 text-center">
        ...
        <p className="mt-6 font-mono text-[9px] tracking-[0.2em] uppercase text-muted-foreground/50">@ModernTechLLC everywhere</p>
      </section>
